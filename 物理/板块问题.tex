...
% 可选：如主要用于送印，可启用印刷模式与审稿章
\MathNoteEnablePrint
\MathNoteEnableReviewStamp

\renewcommand{\notetitle}{高中物理笔记：板块问题}
\renewcommand{\noteauthor}{Gemini AI}
\renewcommand{\notedate}{\today}
\renewcommand{\notesubtitle}{板块模型的情景分类与解题策略}
\renewcommand{\noteversion}{v1.0}

% 物理专用强调色：复用模板主色
\colorlet{FormulaColor}{screenTone07}
\colorlet{HighlightColor}{highlight}
% v-t 图像配色：使用 16 色屏幕色板
\colorlet{vtBlockA}{screenTone03}
\colorlet{vtBlockB}{screenTone13}
\colorlet{vtBoardM}{screenTone07}
\colorlet{vtCommon}{screenTone15}
\colorlet{vtFillA}{screenTone03!20!white}
\colorlet{vtFillB}{screenTone13!20!white}

\begin{document}

% 封面
\thispagestyle{empty}
\PageTag[secondary]{高中物理 · 板块问题}
\setcounter{page}{1}
...
\begin{minipage}{0.84\textwidth}
  ...
  \begin{itemize}
    \item 围绕\keyword{板块模型}这一典型情景，总结“板上滑块 + 外界约束”的常见变式与统一分析框架；
    \item 强调从\inlinehint{相对运动与摩擦类型}入手，结合整体法 / 隔离法与\keyword{v-t 图像}完成多阶段运动刻画；
    \item 通过“光滑地面 / 粗糙地面 / 斜面 / 多滑块”等模块化整理，形成可迁移的\keyword{解题流程}与\keyword{能量视角}。
  \end{itemize}
\end{minipage}
\end{center}
\vspace{1em}
...
\begin{summarybox}{本章小结：板块问题三大视角}
  \begin{focuspoints}
    \item 从\keyword{整体法 + 隔离法}出发统一处理受力与加速度，明确系统与子系统的关系；
    \item 将\keyword{静/动摩擦}视为“相对运动状态”的结果，通过临界静摩擦力判断是否发生滑动；
    \item 借助\keyword{$v\!-\!t$ 图像}将相对位移与摩擦生热几何化，使多阶段运动一图在手、全程可追踪。
  \end{focuspoints}
\end{summarybox}
...
\begin{itemize}
    ...
    \item \textbf{v-t 图像}:
    \begin{center}
        \begin{tikzpicture}[mathnote lines, scale=1.2]
            \draw[->] (0,0) -- (5,0) node[right] {$t$};
            \draw[->] (0,0) -- (0,3) node[above] {$v$};
            \draw[thick, vtBlockA] (0,2.5) node[left] {$v_0$} -- (3,1) node[midway, above] {滑块 $m$};
            \draw[thick, vtBoardM] (0,0) -- (3,1) node[midway, below] {木板 $M$};
            \draw[dashed] (3,1) -- (3,0) node[below] {$t_1$};
            \draw[dashed] (3,1) -- (0,1) node[left] {$v_{共}$};
            \draw[thick, vtCommon] (3,1) -- (4.5,1) node[right] {共同运动};
            \fill[vtFillA, opacity=0.5] (0,0) -- (0,2.5) -- (3,1) -- cycle;
            \fill[vtFillB, opacity=0.5] (0,0) -- (3,1) -- cycle;
...
\begin{itemize}
    ...
    \begin{center}
        \begin{tikzpicture}[mathnote lines, scale=1.2]
            ...
            \draw[->] (0,0) -- (0,3) node[above] {$v$};
            \draw[thick, vtBlockA] (0,2.5) node[left] {$v_0$} -- (2,1) node[midway, above] {$m$ 减速};
            \draw[thick, vtBoardM] (0,0) -- (2,1) node[midway, below] {$M$ 加速};
            \draw[dashed] (2,1) -- (2,0) node[below] {$t_1$};
            \draw[dashed] (2,1) -- (0,1) node[left] {$v_{共}$};
            \draw[thick, vtCommon] (2,1) -- (4,0) node[midway, below right] {共同减速};
        \end{tikzpicture}
    \end{center}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{center}
        \begin{tikzpicture}[mathnote lines, scale=1.2]
            ...
            % Block A
            \draw[thick, vtBlockA] (2.5,2) node[right] {$v_A$} -- (4,0.5) node[midway, above] {$A$};
            % Block B
            \draw[thick, vtBlockB] (2.5,-1) node[left] {$-v_B$} -- (4,0.5) node[midway, below] {$B$};
            % Plate M
            \draw[thick, vtBoardM] (2.5,0) -- (4,0.5) node[midway, above] {$M$};
...
\end{itemize}
\begin{center}
    \begin{tikzpicture}[mathnote lines, scale=1.2, every node/.style={font=\small}]
        % 坐标轴
        ...
        % 物体 A
        \draw[thick, vtBlockA] (0,2) node[right] {$v_A$} -- (4,0) node[midway, above right] {滑块 A};
        
        % 物体 B
        \draw[thick, vtBlockB] (0,-1.5) node[left] {$-v_B$} -- (3,0) node[midway, below right] {滑块 B};
        
        % 物体 M
        \draw[ultra thick, vtBoardM] (0,0) -- (3,0); % 在B停止前
        \draw[thick, vtBoardM] (3,0) -- (4,0); % B停止后与A共速
        \node[text=vtBoardM] at (1.5, 0.2) {木板 M 保持静止};

        % 辅助线与标注
        \draw[dashed] (3,0) -- (3,-2) node[below] {$t_B$};
        \draw[dashed] (4,0) -- (4,-2) node[below] {$t_A$};
        \node[align=center, fill=screenTone09!8!white, rounded corners] at (3.5, -1.5) {B停止后, \\ M受A作用开始运动 (但此例中一起停止)};
    \end{tikzpicture}
...
\begin{center}
    \begin{tikzpicture}[mathnote lines, scale=1.2, every node/.style={font=\small}]
        ...
        % 物体 A
        \draw[thick, vtBlockA] (0,2) node[right] {$v_A$} -- (2.5,0.8);
        
        % 物体 B
        \draw[thick, vtBlockB] (0,-1) node[left] {$-v_B$} -- (2.5,0.8);
        
        % 物体 M
        \draw[thick, vtBoardM] (0,0) -- (2.5,0.8);
        
        % 第二阶段
        \draw[thick, vtCommon, line width=1.5pt] (2.5,0.8) -- (5,0) node[midway, above, sloped] {三者共同减速};
        
        % 标注
        \node[text=vtBlockA] at (1,1.7) {A};
        \node[text=vtBlockB] at (1,-0.3) {B};
        \node[text=vtBoardM] at (1.5,0.2) {M};
...
\begin{center}
    \begin{tikzpicture}[mathnote lines, scale=1.2, every node/.style={font=\small}]
        ...
        % 物体 A - 整个过程加速度不变
        \draw[thick, vtBlockA] (0,2) node[right] {$v_A$} -- (4,1);
        
        % 物体 B
        \draw[thick, vtBlockB] (0,-0.8) node[left] {$-v_B$} -- (1.5,0); % B在t_B时刻停止
        
        % 物体 M - 斜率发生变化
        \draw[thick, vtBoardM] (0,0) -- (1.5,0.4) node[midway, below, sloped] {$a_{M1}$}; % B停止前
        \draw[thick, vtBoardM] (1.5,0.4) -- (4,1) node[midway, below, sloped] {$a_{M2} > a_{M1}$}; % B停止后
        \node[HighlightColor] at (1.5,0.4) {\textbullet}; % 突变点
        \node[HighlightColor, above] at (1.5,0.4) {突变点};

        % 第三阶段
        \draw[thick, vtCommon, line width=1.5pt] (4,1) -- (6,0) node[midway, above, sloped] {A+M 共同减速};
...
\begin{NoteRoadmap}{板块问题通用解题流程}
  \RoadmapStep{明确研究对象与阶段：搞清楚有几个物体、各自初始状态，以及哪些事件会触发“阶段切换”（共速、静止、滑出等）。}
  \RoadmapStep{为每个阶段完成\keyword{受力分析}：画清受力图，特别是各接触面上的摩擦力方向与大小的可能范围。}
  \RoadmapStep{判断相对运动与摩擦类型：通过比较“所需摩擦力”和“最大静摩擦力”，或直接比较各物体加速度，区分相对静止与相对滑动。}
  \RoadmapStep{选取合适规律：结合牛顿第二定律、运动学公式、动量守恒（条件满足时）及能量守恒 / 功–能定理，求出目标物理量。}
  \RoadmapStep{用 $v\!-\!t$ 图像收束：用斜率和面积统一理解速度、位移与相对位移，检验前面各阶段的分析是否自洽。}
\end{NoteRoadmap}

\begin{summarybox}{“一个临界，两种观点”}
  \begin{focuspoints}
    \item \keyword{临界点}: “恰好”发生相对滑动时静摩擦力达到最大值，是求最大拉力、最小板长等问题的突破口；
    \item \keyword{动力学观点}: 以 $F=ma$ 为核心，关注每一时刻力与加速度的对应关系；
    \item \keyword{能量观点}: 用 $W_{\text{net}} = \Delta E_k$ 与 $E_{\text{系统}} = E_k + Q$ 描述整体过程，特别适合处理总路程与摩擦生热等结果量，避免纠缠于各阶段的具体时间与加速度。
  \end{focuspoints}
  \begin{center}
    \(\textcolor{FormulaColor}{Q_{\text{摩擦生热}} = f_{\text{滑动摩擦力}} \times s_{\text{相对位移}}}\)
  \end{center}
\end{summarybox}
