...
\ModeBadge[secondary]{情形 I：$0 < v_0 \le \sqrt{2gR}$}
...
\ModeBadge[highlight]{情形 II：$\sqrt{2gR} < v_0 < \sqrt{5gR}$}
...
\begin{summarybox}{离轨后的抛体运动统一模型}
  设小球在 $P(\theta_1)$ 处以速度 $v_1$ 离开轨道。以 $P$ 为原点，水平向右为 $x$ 轴、竖直向上为 $y$ 轴，则：
  \begin{itemize}
    \item 离轨点相对于圆心 $O$ 的坐标为
          \[
            P:\ (x_P,y_P) = (R\sin\theta_1,\,-R\cos\theta_1);
          \]
    \item 切线方向单位向量
          \[
            \mathbf{t} = (\cos\theta_1,\ \sin\theta_1),\quad
            \vec v_1 = v_1\mathbf{t};
          \]
    \item 以 $P$ 为原点的抛体轨迹方程为
          \[
            x(t) = v_1\cos\theta_1\,t,\qquad
            y(t) = v_1\sin\theta_1\,t - \frac12 gt^2.
          \]
  \end{itemize}
  由前文 $N=0$ 条件知
  \[
  ...
  因此竖直方向初速度分量与最大高度分别为
  \[
    v_{1y} = v_1\sin\theta_1,\qquad
    H_{\max} = \frac{v_{1y}^2}{2g}
            = \frac{v_1^2\sin^2\theta_1}{2g}.
  \]
  常用结论：
  \[
    t_{\text{回到离轨高度}} = \frac{2v_1\sin\theta_1}{g},\qquad
    L_{\text{水平位移}} = \frac{2v_1^2\sin\theta_1\cos\theta_1}{g}.
  \]
\end{summarybox}

\begin{center}
\begin{tikzpicture}[mathnote lines, scale=1.05]
  \def\R{1.6}
  \coordinate (O) at (0,0);
  \draw[accent, thick] (O) circle (\R);

  \def\thetaone{130}
  \coordinate (P) at ({\R*sin(\thetaone)}, -{\R*cos(\thetaone)});
  \fill (O) circle (0.5pt) node[below right=1pt] {$O$};
  \fill (P) circle (1.1pt) node[above left=-1pt] {$P(\theta_1)$};

  % 离轨点的切线方向速度
  \draw[highlight, -{Latex[length=2mm]}] (P) -- ++({0.9*cos(\thetaone)},{0.9*sin(\thetaone)}) node[above right] {$\vec v_1$};

  % 抛物线大致轨迹
  \draw[secondary, thick, domain=0:1.4,samples=60]
    plot ({\R*sin(\thetaone) + 1.0*\x},
          {-\R*cos(\thetaone) + 0.9*sin(\thetaone)*\x - 0.8*\x*\x});

  % 最高点 C 对应高度示意（虚线）
  \coordinate (C) at (0,\R);
  \fill (C) circle (0.9pt) node[above] {$C$};
  \draw[densely dashed, inkgray] (P) -- ++(0,{1.2*\R});
  \draw[densely dashed, inkgray] (C) -- ++({1.1*\R},0);
\end{tikzpicture}
\captionof{figure}{离轨点处的抛体运动示意}
\end{center}

\begin{summarybox}{一条有趣的结论：抛体高度小于轨道最高点}
  ...
  而抛体的最大高度为
  \[
    H_{\max} = \frac{v_1^2\sin^2\theta_1}{2g}
             = -\frac{R}{2}\cos\theta_1(1-\cos^2\theta_1).
  \]
  在 $\tfrac{\pi}{2}<\theta_1<\pi$ 区间内有 $-1<\cos\theta_1<0$，可证明
  \[
    H_{\max} < h_{C},
  \]
  即\keyword{离轨后的小球抛起的最高点总是低于圆轨道的最高点}。因此，当 $v_0<\sqrt{5gR}$ 时，小球不可能飞越轨道顶端。
\end{summarybox}
...
\ModeBadge[secondary]{情形 III：$v_0 \ge \sqrt{5gR}$}
