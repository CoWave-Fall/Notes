...
\begin{minipage}{0.84\textwidth}
  ...
  \begin{itemize}
    \item 从\keyword{单位圆}与\keyword{三角函数线}出发，建立任意角三角函数的统一几何模型；
    \item 系统整理\inlinehint{同角三角函数关系}、\inlinehint{诱导公式}与\inlinehint{三角恒等变换}等核心公式族；
    \item 通过图像、恒等式与不等式三条路径，构建解决\keyword{三角函数方程与不等式}的常用思路。
  \end{itemize}
\end{minipage}
\end{center}
\vspace{1em}
...
\tableofcontents
\begin{roadmap}
  \RoadmapStep{理解任意角三角函数的定义与单位圆表示。}
  \RoadmapStep{熟练掌握同角关系与诱导公式的变形与逆用。}
  \RoadmapStep{从图像角度把握函数的周期性、单调性与对称性。}
  \RoadmapStep{综合运用恒等变换解决方程与不等式问题。}
\end{roadmap}
\newpage
...
\begin{tikzpicture}[scale=2.5]
    ...
    % 三角函数线 (标签位置已修正)
    \draw[highlight, very thick] (M) -- (P) node[midway, right=4pt, color=highlight] {$\sin\alpha$};
    \draw[secondary, very thick] (0,0) -- (M) node[midway, below=2pt, color=secondary] {$\cos\alpha$};
    
    % 辅助线 (标签位置已修正)
    \draw[->, thick, accent] (0.5,0) arc (0:\angle:0.5cm) node[midway, right] {$\alpha$};
    ...
    \draw[accent, very thick] (1,0) -- (T) node[midway, right=2pt, color=accent] {$\tan\alpha$};
    \draw[dashed] (P) -- (T);
...
\begin{focuspoints}
  \item 用单位圆统一记忆：$P(\cos\alpha,\sin\alpha)$，横坐标对应 $\cos\alpha$，纵坐标对应 $\sin\alpha$；
  \item 六个三角函数只由 $\sin\alpha$ 与 $\cos\alpha$ 两个量决定，其余四个都是商或倒数关系；
  \item 通过 $r=1$ 的单位圆，可以把直角三角形中的边比定义自然推广到任意实数角。
\end{focuspoints}
...
\begin{summarybox}{同角关系的常见化简思路}
  \begin{itemize}
    \item 遇到 $\sin^2\alpha$ 与 $\cos^2\alpha$ 同时出现时，优先尝试用 $\sin^2\alpha + \cos^2\alpha = 1$ 降低幂次或消元；
    \item 含有 $\tan\alpha$、$\cot\alpha$ 时，常用商关系 $\tan\alpha=\dfrac{\sin\alpha}{\cos\alpha}$ 把式子统一到 $\sin\alpha,\cos\alpha$；
    \item 出现 $\sec\alpha,\csc\alpha$ 等倒数函数时，可以先用倒数关系转回 $\cos\alpha,\sin\alpha$，再与平方、商关系结合使用。
  \end{itemize}
\end{summarybox}
...
\begin{center}
\begin{tikzpicture}[mathnote lines, domain=-2.2*pi:2.2*pi, samples=200, scale=1]
    % Axes
    ...
    \draw[accent, thick, smooth] plot (\x, {sin(\x r)});
    \node[accent, below right] at (0.5*pi, 1) {$y=\sin x$};
    
    \draw[secondary, thick, smooth, dashed] plot (\x, {cos(\x r)});
    \node[secondary, above] at (0, 1) {$y=\cos x$};
\end{tikzpicture}
...
\begin{tikzpicture}[mathnote lines, scale=1]
    ...
    % Asymptotes
    \draw[dashed, inkgray!35] (-1.5*pi, -4) -- (-1.5*pi, 4);
    \draw[dashed, inkgray!35] (-0.5*pi, -4) -- (-0.5*pi, 4);
    \draw[dashed, inkgray!35] (0.5*pi, -4) -- (0.5*pi, 4);
    \draw[dashed, inkgray!35] (1.5*pi, -4) -- (1.5*pi, 4);
    ...
    \draw[highlight, thick, smooth, domain=-1.5*pi+0.02:-0.5*pi-0.02, samples=100] plot (\x, {tan(\x r)});
    \draw[highlight, thick, smooth, domain=-0.5*pi+0.02:0.5*pi-0.02, samples=100] plot (\x, {tan(\x r)});
    \draw[highlight, thick, smooth, domain=0.5*pi+0.02:1.5*pi-0.02, samples=100] plot (\x, {tan(\x r)});
    
    \node[color=highlight, right] at (0.8, {tan(0.8 r)}) {$y=\tan x$};
\end{tikzpicture}
\captionof{figure}{正切函数图像}
\end{center}

\begin{summarybox}{正弦、余弦与正切函数的图像特征}
  \begin{itemize}
    \item \keyword{定义域与值域}：$\sin x,\cos x$ 在整个 $\R$ 上有定义，值域为 $[-1,1]$；$\tan x$ 在 $x\neq \dfrac{\pi}{2}+k\pi$ 上有定义，值域为 $\R$；
    \item \keyword{周期性}：$\sin x,\cos x$ 的基本周期为 $2\pi$，$\tan x$ 的基本周期为 $\pi$，在解题时可用“平移一个周期不变”简化区间；
    \item \keyword{奇偶性与对称性}：$\sin x$ 为奇函数（关于原点对称），$\cos x$ 为偶函数（关于 $y$ 轴对称），$\tan x$ 也是奇函数；
    \item \keyword{单调区间}：常用区间为 $[-\dfrac{\pi}{2},\dfrac{\pi}{2}]$ 上 $\sin x$ 递增、$[0,\pi]$ 上 $\cos x$ 递减、$\left(-\dfrac{\pi}{2},\dfrac{\pi}{2}\right)$ 上 $\tan x$ 递增。
  \end{itemize}
\end{summarybox}
...
\begin{enumerate}
    ...
    \begin{center}
    \begin{tikzpicture}[mathnote lines, domain=-pi/2:2*pi, samples=200, scale=1.2]
        \draw[->] (-pi/2-0.2, 0) -- (2*pi+0.2, 0) node[right] {$x$};
        \draw[->] (0,-1.2) -- (0,1.2) node[above] {$y$};
        
        \draw[accent, thick] plot (\x, {sin(\x r)});
        \draw[secondary, dashed, thick] plot (\x, {cos(\x r)});
        
        \fill[accent!20, domain=pi/4:5*pi/4] plot(\x, {sin(\x r)}) -- (5*pi/4, {cos(5*pi/4 r)}) -- plot[domain=5*pi/4:pi/4] (\x, {cos(\x r)}) -- (pi/4, {sin(pi/4 r)});
        ...
        \node at (pi*1.2, 1) {\color{accent}\Large $\sin x > \cos x$};
    \end{tikzpicture}
    \end{center}
    ...
    \begin{center}
    \begin{tikzpicture}[mathnote lines, scale=1.5]
        \node at (0.8, {tan(0.8 r)}) [color=highlight, right] {$ y=\tan x $};
        \draw[->] (-1.5,0) -- (1.5,0) node[right] {$x$};
        \draw[->] (0,-1.5) -- (0,1.5) node[above] {$y$};
        \draw[thin, inkgray!35] (0,0) circle (1);
        \draw[dashed] (-1.2, -1.2) -- (1.2, 1.2) node[right] {$y=x$};
        
        \coordinate (A) at (45:1);
        \coordinate (B) at (225:1);
        
        \draw[accent, very thick] (A) arc (45:225:1);
        \node[color=accent, below right] at (A) {$P(\cos x, \sin x)$};
        \node at (-1, 1) {\color{accent}\Large $y > x$};
