...
% ==================================================
\renewcommand{\notetitle}{端点/内点效应高中数学参考笔记}
\renewcommand{\noteauthor}{高中数学参考笔记}
\renewcommand{\notedate}{\today}
\renewcommand{\notesubtitle}{导数压轴·端点效应与内点效应}
\renewcommand{\noteversion}{v1.0}
...
% 封面
\thispagestyle{empty}
\PageTag[secondary]{高中数学·端点/内点效应}
\setcounter{page}{1}
...
\begin{minipage}{0.84\textwidth}
  ...
  \begin{itemize}
    \item 围绕\keyword{端点效应}与\keyword{内点效应}两大核心概念，梳理闭区间最值、导数单调性等典型模型；
    \item 通过\inlinehint{端点比较}与\inlinehint{驻点分析}的对比，构建一套可迁移的\keyword{统一解题框架}；
    \item 配套基础到压轴的例题与高考真题，帮助形成系统的\keyword{参数讨论}与\keyword{综合判断}能力。
  \end{itemize}
\end{minipage}
\end{center}
\vspace{1em}
...
端点效应和内点效应是高中数学导数部分的核心概念，也是高考数学导数压轴题的重要解题方法。本笔记采用“理论深入 + 方法对比 + 例题丰富 + 应用综合”的体系，帮助同学们从整体上把握端点/内点效应在各类题型中的作用。

\chapter{理论基础}

\section{核心概念定义}

\begin{definitionbox}{核心概念}
...
\begin{theorembox}{费马引理}
设函数 $f(x)$ 在点 $x_0$ 处可导，且在 $x_0$ 处取得极值，则 $f'(x_0) = 0$。

\textbf{注意}：费马引理仅适用于内点，对于端点处的极值需要单独分析。
\end{theorembox}

\begin{theorembox}{极值存在性定理}
设函数 $f(x)$ 在闭区间 $[a,b]$ 上连续，则 $f(x)$ 在 $[a,b]$ 上必有最大值和最小值。
...
\textbf{选择原则：}
\begin{itemize}
    \item 当函数在端点处有明显特征时，优先考虑\keyword{端点效应}
    \item 当函数在内部有驻点时，重点分析\keyword{内点效应}
    \item 复杂问题需要\keyword{综合运用}两种方法
\end{itemize}
...
\begin{examplebox}{例题1（基础）}
已知函数 $f(x) = ax^2 + bx + c$，当 $x \in [0, 1]$ 时，$f(x) \geq 0$ 恒成立，求参数 $a, b, c$ 的取值范围。
...
\begin{examplebox}{例题2（提高）}
已知函数 $f(x) = \ln x - ax$，当 $x \geq 1$ 时，$f(x) \leq 0$ 恒成立，求参数 $a$ 的取值范围。
...
\begin{examplebox}{例题3（中等）}
已知函数 $f(x) = x^3 - 3x + a$，问是否存在实数 $a$，使得 $f(x)$ 在区间 $[0, 2]$ 上有两个不同的零点？
...
\begin{examplebox}{例题4（较难）}
已知函数 $f(x) = e^x - ax - 1$，当 $x \geq 0$ 时，$f(x) \geq 0$ 恒成立，求参数 $a$ 的取值范围。
...
\begin{examplebox}{例题5（基础）}
求函数 $f(x) = x^4 - 4x^3 + 4x^2$ 在区间 $[0, 3]$ 上的最大值和最小值。
...
\begin{examplebox}{例题6（提高）}
证明当 $x \in [0, 1]$ 时，$x - x^2 \leq \frac{1}{4}$。
...
\begin{examplebox}{2023年全国甲卷}
已知函数 $f(x) = ax - \frac{a}{x} - 2\ln x$，$a > 0$。
\begin{enumerate}
