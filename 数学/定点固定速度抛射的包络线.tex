...
% 专用配色：从 mathnote 16 色主题抽取
\colorlet{myblue}{screenTone03}
\colorlet{mygreen}{screenTone07}
\colorlet{myred}{screenTone13}
\colorlet{myorange}{screenTone12}
\colorlet{mypurple}{screenTone15}

\begin{document}

% 封面
\thispagestyle{empty}
\PageTag[secondary]{高中数学·抛射运动包络线}
\setcounter{page}{1}
...
\begin{minipage}{0.84\textwidth}
  ...
  \begin{itemize}
    \item 围绕\keyword{定点固定速度抛射}这一典型物理模型，刻画不同抛射角轨迹构成的\keyword{曲线族}与其\keyword{包络线}；
    \item 在\inlinehint{物理运动分解}与\inlinehint{曲线族求包络线}之间建立对应，形成可迁移的\keyword{“物理情景 $\rightarrow$ 数学模型 $\rightarrow$ 包络线”}流程；
    \item 通过例题与判别式方法，训练对\keyword{可达区域}、\keyword{最优抛射角}等问题的统一分析框架。
  \end{itemize}
\end{minipage}
\end{center}
\vspace{1em}
...
\begin{tikzpicture}[scale=4.0, x=2.5cm, y=4cm, >=Stealth]
    ...
    % 关键点标注（最高点与落地点圆点略小）
    \fill[myred] (0.5*\xmax, \ymax) circle (0.6pt);
    ...
    \fill[myred] (\xmax, 0) circle (0.6pt);
...
\end{enumerate}
则称 $C$ 为该曲线族的\keyword{包络线}。

求包络线的一般步骤可以用下面的\textbf{数学路线图}来概括：

\begin{roadmap}
  \RoadmapStep{写出曲线族的方程 $F(x,y,\alpha)=0$，明确参数 $\alpha$ 的含义（本问题中是抛射角或其正切）。}
  \RoadmapStep{对参数求偏导：令 $\dfrac{\partial F}{\partial \alpha}(x,y,\alpha)=0$，刻画“相切”的条件。}
  \RoadmapStep{联立
  \[
    \begin{cases}
      F(x,y,\alpha)=0,\\[0.3em]
    ...
    \end{cases}
  \]
  消去参数 $\alpha$，得到仅含 $x,y$ 的方程。}
  \RoadmapStep{将得到的曲线理解为原曲线族的“边界”，并结合物理情景解释其几何意义。}
\end{roadmap}
\end{definitionbox}

\subsection{利用参数法求包络线}

对于抛体运动的轨迹族，我们使用 $k = \tan\theta$ 作为参数会更方便。设
\[
  F(x,y,k) = y - xk + \frac{gx^2}{2v_0^2}(1 + k^2) = 0, \label{eq:F}
\]
这正是轨迹方程 \eqref{eq:family} 的等价形式。

根据上面的路线图：
\begin{focuspoints}
  \item 将\inlinehint{几何条件“相切”}翻译为关于参数 $k$ 的\keyword{重根条件}；
  \item 用 $\dfrac{\partial F}{\partial k}=0$ 刻画“重根”或“切点”，再与 $F=0$ 联立；
  \item 消去 $k$ 后得到的曲线，就是这族抛物线的包络线。
\end{focuspoints}

对参数 $k$ 求偏导：
\[
  \frac{\partial F}{\partial k} = -x + \frac{gx^2}{2v_0^2} \cdot 2k
  = -x + \frac{gx^2}{v_0^2}k = 0.
\]
由此可得
\[
  -x + \frac{gx^2k}{v_0^2} = 0
  \quad\Longrightarrow\quad
  x\!\left(-1 + \frac{gxk}{v_0^2}\right) = 0.
\]
当 $x = 0$ 时对应抛射点（不是我们关心的包络线上的一般点），因此必须有
\[
  -1 + \frac{gxk}{v_0^2} = 0
  \quad\Longrightarrow\quad
  k = \frac{v_0^2}{gx}. \label{eq:k}
\]

将上式代入 $F(x,y,k)=0$：
\begin{align}
  y
  &= x \cdot \frac{v_0^2}{gx}
     - \frac{gx^2}{2v_0^2}\left(1 + \left(\frac{v_0^2}{gx}\right)^2\right) \nonumber\\
  &= \frac{v_0^2}{g}
     - \frac{gx^2}{2v_0^2}\left(1 + \frac{v_0^4}{g^2x^2}\right) \nonumber\\
  &= \frac{v_0^2}{g}
     - \frac{gx^2}{2v_0^2}
     - \frac{v_0^2}{2g} \nonumber\\
  &= \frac{v_0^2}{2g}
...
这是一个开口向下的抛物线，其性质为：
\begin{itemize}
  \item 顶点：$\left(0, \dfrac{v_0^2}{2g}\right)$，对应竖直上抛能达到的最大高度；
  \item 对称轴：$x = 0$（$y$ 轴），反映抛体运动关于竖直方向的对称性；
  \item 与 $x$ 轴的交点：$x = \pm \dfrac{v_0^2}{g}$，这是水平抛射的最大射程。
\end{itemize}
\end{theorembox}

\subsection{判别式视角：一次性判定可达区域}

还可以从\keyword{判别式}的角度理解包络线。

把轨迹方程 \eqref{eq:trajectory} 看作关于 $k=\tan\theta$ 的二次方程：
\begin{equation}
  \frac{gx^2}{2v_0^2}k^2 - xk
  + \left(y + \frac{gx^2}{2v_0^2}\right) = 0.
\end{equation}
若给定点 $(x, y)$ 处存在实数抛射角使质点经过，则上述方程对 $k$ 有\keyword{实根}，即判别式
\[
  \Delta \geq 0.
\]
...
\begin{align}
  \Delta
  &= x^2
   - 4 \cdot \frac{gx^2}{2v_0^2}
     \left(y + \frac{gx^2}{2v_0^2}\right) \nonumber\\
  &= x^2
   - \frac{2gx^2}{v_0^2}\left(y + \frac{gx^2}{2v_0^2}\right) \nonumber\\
  &= x^2
   - \frac{2gx^2y}{v_0^2}
   - \frac{g^2x^4}{v_0^4}.
\end{align}
...
\begin{summarybox}{判别式视角下的“可达区域”}
\begin{itemize}
  \item \keyword{包络线} $y = \dfrac{v_0^2}{2g} - \dfrac{gx^2}{2v_0^2}$ 对应 $\Delta = 0$：给定点恰好只有一个抛射角（重根），轨迹与包络线相切；
  \item \keyword{包络线内部}对应 $\Delta > 0$：存在两个抛射角（两根），通常是“低抛/高抛”两种方案；
  \item \keyword{包络线外部}对应 $\Delta < 0$：无实根，说明无论怎样选择抛射角都无法到达该点。
\end{itemize}
\end{summarybox}

这给出了一个\keyword{一眼判断可达性}的工具：只需把点坐标代入包络线不等式，就能判断是否存在合适抛射角。
...
\begin{summarybox}{固定速度抛射包络线·知识小结}
\begin{itemize}
  \item \keyword{抛体运动模型}：从定点以初速度 $v_0$、抛射角 $\theta$ 抛出质点，运动可分解为水平方向匀速、竖直方向匀加速；
  \item \keyword{轨迹族方程}：
  \[
    y = x\tan\theta - \frac{gx^2}{2v_0^2}(1+\tan^2\theta),
  \]
  固定 $v_0$、令 $\theta$ 变化即可得到一族抛物线；
  \item \keyword{包络线方程}：
  \[
    y = \frac{v_0^2}{2g} - \frac{gx^2}{2v_0^2},
  \]
  是所有轨迹的几何“边界”，刻画\inlinehint{可达区域}与\inlinehint{不可达区域}；
  \item \keyword{两种求法统一}：一是用 $F(x,y,k)=0$ 与 $\dfrac{\partial F}{\partial k}=0$ 消去参数，二是用判别式 $\Delta\ge0$ 描述“有无抛射角”；
  \item \keyword{常见问题类型}：点的可达性判断、最优抛射角、最大射程/最大高度、给定目标点反求初速度或角度等。
\end{itemize}
\end{summarybox}

包络线问题把曲线族理论与经典力学中的抛体运动紧密结合，是数理结合的典型模型。熟练掌握“\keyword{写出轨迹族} $\rightarrow$ \keyword{求包络线} $\rightarrow$ \keyword{解释可达区域}”的统一流程，可以自然迁移到更多含参曲线族与极值边界类问题中。
