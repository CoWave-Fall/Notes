...
\providecommand{\MathNoteReviewStampBase}{../assest/lzlxV-reviewed}
...
\makeatletter
\newtcolorbox{methodbox}[2][]{%
  mathnote box,
  title=\mathnote@ifblank{#2}{方法}{#2},
  colback=surface,
  colframe=accent!70!black,
  coltitle=accent!15!white,
...
\renewcommand{\notetitle}{偏导数思想在含参导数问题中的应用}
\renewcommand{\noteauthor}{Gemini 数学组}
\renewcommand{\notedate}{\today}
\renewcommand{\notesubtitle}{含参导数题型的统一分析框架}
\renewcommand{\noteversion}{v1.0}

\begin{document}

\thispagestyle{empty}
\PageTag[secondary]{含参导数}
\setcounter{page}{1}
...
\begin{minipage}{0.84\textwidth}
  \textcolor{inkgray}{\sffamily\small 研读提示}\\
  \begin{itemize}
    \item 将含参函数视作二元函数，利用 \keyword{偏导数} 统一单调性、极值与最值的分析路径；
    \item 通过 \ModeBadge{方法盒} 与 \ModeBadge[highlight]{典型例题} 串联「概念 $\rightarrow$ 模型 $\rightarrow$ 题型」；
    \item 在每个专题节点加入参数视角的 \inlinehint{分类讨论}，帮助建立更具拓展性的复习框架。
  \end{itemize}
\end{minipage}
\end{center}
\vspace{1em}
...
含参函数：形如 $f(x,a)$ 的函数，其中 $x$ 是自变量，$a$ 是参数。从偏导数的角度看，我们可以将 $a$ 也视为变量，这样 $f(x,a)$ 就是一个二元函数。
\end{conceptbox}
...
\section{单调性问题中的偏导数思想}

\subsection{对x求导分析单调性}
...
\begin{enumerate}
    ...
    \item 分析：对任意 $x \neq a$，有 $\partialder{f}{x} = 3(x-a)^2 > 0$，仅在 $x = a$ 处取零，因此 $f(x)$ 在整个实数范围上单调递增
    \item 结论：$x = a$ 是驻点，但不是极值点（这是只有驻点而无极值的典型例子）
\end{enumerate}
\end{examplebox}

\subsection{二阶导数判断极值性质}
...
\begin{enumerate}
    ...
    \end{itemize}
    \item 由于 $\partialder{f}{x} = 3(x-a)^2 \geq 0$ 对所有 $x$ 成立，$f(x)$ 在 $[0,2]$ 上单调递增，因此
    \begin{itemize}
        \item 最小值为 $f_{\min} = f(0) = 0$；
        \item 最大值为 $f_{\max} = f(2) = 2(4 - 6a + 3a^2)$。
    \end{itemize}
\end{enumerate}
\end{examplebox}
...
\subsection{f(x) \texorpdfstring{$\geq$}{>=} 0恒成立问题}
...
\textbf{解：}
\begin{enumerate}
    \item 当 $x = 0$ 时，$x^2 + ax + 1 = 1 \geq 0$ 恒成立；下面只需考虑 $x \in (0,1]$
    \item 参变分离：$x^2 + ax + 1 \geq 0 \Leftrightarrow ax \geq -x^2 - 1$，即 $a \geq -\frac{x^2 + 1}{x} = -x - \frac{1}{x}$
    \item 构造函数 $g(x) = -x - \frac{1}{x}$，定义域为 $x \in (0,1]$
    \item 计算导数：$g'(x) = -1 + \frac{1}{x^2}$
    \item 在 $(0,1)$ 内有 $x^2 < 1$，于是 $g'(x) > 0$，故 $g(x)$ 在 $(0,1]$ 上单调递增，其上确界为 $g(1) = -2$
    \item 要使不等式对所有 $x \in [0,1]$ 恒成立，需 $a \geq \sup_{x \in (0,1]} g(x) = -2$，即 $a \geq -2$
\end{enumerate}
\end{examplebox}

\subsection{最值法求解恒成立问题}
...
\begin{enumerate}
    ...
    \item 设 $t = \sqrt{a} \geq 0$，则条件等价于 $1 - 2t^3 \geq 0$，即 $t^3 \leq \dfrac{1}{2}$，从而 $t \leq \sqrt[3]{\dfrac{1}{2}}$，于是 $a = t^2 \leq \left(\dfrac{1}{2}\right)^{\frac{2}{3}} = \dfrac{1}{\sqrt[3]{4}}$
    \item 综合：$a \leq \dfrac{1}{\sqrt[3]{4}}$
\end{enumerate}
\end{examplebox}
...
\subsection{存在x使f(x) \texorpdfstring{$\geq$}{>=} 0成立}
