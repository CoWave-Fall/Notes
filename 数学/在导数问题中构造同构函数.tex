...
% 如以打印版为主，可保留；若以屏幕阅读为主，可按需注释掉
\MathNoteEnablePrint
\MathNoteEnableReviewStamp

% ==================================================
% Math helpers specific to this note
% ==================================================
% 导数符号
...
% 图标式标签：使用模板提供的 ModeBadge，而非额外图标字体
\newcommand{\exampleicon}{\ModeBadge[highlight]{例题}}
\newcommand{\analysisicon}{\ModeBadge[secondary]{分析}}
\newcommand{\revisionicon}{\ModeBadge[accent]{整理}}
\newcommand{\examicon}{\ModeBadge[accent]{考点}}
\newcommand{\tipicon}{\ModeBadge[secondary]{提示}}
\newcommand{\warningicon}{\ModeBadge[highlight]{注意}}
\newcommand{\keyicon}{\ModeBadge[accent]{关键}}

% 方法与提示盒：复用 mathnote box 统一风格
\makeatletter
...
\newtcolorbox{tipbox}[2][]{%
  ...
  colback=surface,
  colframe=secondary!60!black,
  coltitle=secondary!15!white,
  fonttitle=\sffamily\bfseries\color{secondary!45!white},
...
% ==================================================
\renewcommand{\notetitle}{在导数问题中构造同构函数}
\renewcommand{\noteauthor}{高中数学参考笔记}
\renewcommand{\notedate}{\today}
\renewcommand{\notesubtitle}{同构思想在导数问题中的系统应用}
\renewcommand{\noteversion}{v1.0}
...
\begin{minipage}{0.84\textwidth}
  ...
  \begin{itemize}
    \item 围绕\keyword{同构函数}这一核心工具，串联导数题目中的单调性、极值与不等式证明模型；
    \item 强调从\inlinehint{结构相似}出发识别同构，再通过\inlinehint{构造与验证}完成求解；
    \item 配套概念梳理、方法盒与典型例题，帮助建立可迁移的\keyword{同构模型库}与\keyword{统一分析框架}。
  \end{itemize}
\end{minipage}
\end{center}
\vspace{1em}
