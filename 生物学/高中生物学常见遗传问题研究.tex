...
% TikZ 附加库（在 mathnote 已加载 tikz 的基础上扩展）
\usetikzlibrary{intersections,quotes,shapes.geometric,matrix}

% ============================================
% MathNote 元信息与自定义命令
% ============================================

% 笔记元信息（用于封面、页眉与 PDF 元数据）
\renewcommand{\notetitle}{高中生物学常见遗传问题研究}
\renewcommand{\noteauthor}{生物学笔记}
\renewcommand{\notedate}{\today}
\renewcommand{\notesubtitle}{常见遗传问题与解题策略}
\renewcommand{\noteversion}{v1.0}

% 可根据输出介质按需启用
% \MathNoteEnablePrint      % 若主要用于印刷，可解除注释
% \MathNoteEnableReviewStamp% 若需要在首页叠加审稿章，可解除注释

% 生物学专用标记
\newcommand{\genotype}[1]{\textcolor{accent}{\textbf{#1}}}   % 基因型高亮
\newcommand{\phenotype}[1]{\textcolor{highlight}{\textbf{#1}}} % 表现型高亮
...
\begin{minipage}{0.84\textwidth}
  ...
  \begin{itemize}
    \item 从\keyword{基因型—表现型}、配子法与概率入手，统一整理高中常见遗传模型；
    \item 以\keyword{孟德尔定律}、\keyword{基因互作}、\keyword{连锁互换} 和 \keyword{伴性遗传} 为主线，总结常见题型与结构；
    \item 配套典型例题、速查表与\inlinehint{易错点清单}，形成可迁移的\keyword{解题步骤}与\keyword{数量分析框架}。
  \end{itemize}
\end{minipage}
\end{center}
\vspace{1em}
